...
/*!
	@brief	This method MUST use <tt>-valueForKey:</tt> to extract the |name|, |type| and |domain| as documented in the <tt>AFNetServiceCommon</tt> description.
 */
...
/*!
	@brief	This method is optional, though it should simply be a concatenation of the |name|, |type| and |domain| suitable for resolution.
 */
- (NSString *)fullName;

/*!
	@brief	This is the expanded form of <tt>-initWithNetService:</tt> taking explict arguments.
 */
...
/*!
	@brief	Converts a data object containing TXT record to a dictionay.
	@detail	The dictionary returned by the <tt>+[NSNetService dictionaryFromTXTRecordData:]</tt> only converts the keys to UTF-8 encoded NSStrings, this function converts the data objects as UTF-8 strings too.
...
/*!
    @brief	A replacement for a resolvable NSNetService with a KVO compliant 'presence' dictionary corresponding to the TXT record data
	@detail	The initialisers for this class are in <tt>AFNetServiceCommon</tt>.
...
/*!
	@brief	This starts observing the TXT record of the service. Interested parties will be notified using the KVO compliant |persence| dictionary property
 */
...
/*!
	@brief	If one of the TXT dicrionary keys has a knock-on effect, like the phsh key for P2P XMPP documented in XEP-0174, you can detect that in an overridden implementation.
...
/*!
	@brief	This returns an array of NSData objects wrapping a (struct sockaddr) suitable for connecting to.
 */
- (NSArray *)addresses;

/*!
    @brief   This will stop both a monitor and resolve operation,
*/
- (void)stop;
