...
/*!
	@brief	This is the parent class for all the network stack objects. You are unlikely to use it directly.
	@detail	This class configures a bidirectional proxying system. Unimplemented methods are forwarded to the |lowerLayer|, and the delegate accessor returns a proxy that forwards messages up the delegate chain.
...
/*!
	@brief	You must override this method if you want to use the designated outbound initialisers.
			The default implementation raises an exception.
 */
+ (Class)lowerLayerClass DEPRECATED_ATTRIBUTE;

/*!
	@brief	This initialiser is a sibling to <tt>-initWithNetService:</tt>.
...
/*!
	@brief	This initialiser is a sibling to <tt>-initWithSignature:</tt>.
	@detail	A net service - once resolved - encapsulates all the data from <tt>AFSocketPeerSignature</tt>.
			The default implementation creates a lower-layer using <tt>+lowerLayerClass</tt> and calls the same initialiser on the new object.
...
/*!
	@brief	This isn't currently used by the framework, it is intended for use like <tt>-[NSThread threadDictionary]</tt> to store miscellaneous data.
 */
